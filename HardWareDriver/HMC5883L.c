...
void  HMC58X3_newValues(int16_t x,int16_t y,int16_t z)
{
	...
	if(HMC5883_calib){//校正有效的话 采集标定值
		...
		LED_Set_Blink(LED2,30,50,1);  //指示 正在标定
	}
...
{ 
  ...
  HMC5883_Offset_X = Config.Magnetic_offset_X ;  //取磁力计的标定值来用
  HMC5883_Offset_Y = Config.Magnetic_offset_Y ; 
  HMC5883_Offset_Z = Config.Magnetic_offset_Z ; 
...
void HMC5883L_Save_Calib(void){
	//将磁力计标定值写入 Flash 保存
	Config.Magnetic_offset_X = HMC5883_Offset_X = (HMC5883_maxx+HMC5883_minx)/2;
	Config.Magnetic_offset_Y = HMC5883_Offset_Y = (HMC5883_maxy+HMC5883_miny)/2;
	Config.Magnetic_offset_Z = HMC5883_Offset_Z = (HMC5883_maxz+HMC5883_minz)/2;

	Config.Magnetic_Scale_X = HMC5883_Scale_X = 1.0f;
	Config.Magnetic_Scale_Y = HMC5883_Scale_Y = (float)(HMC5883_maxx - HMC5883_minx)/(float)(HMC5883_maxy - HMC5883_miny);
	Config.Magnetic_Scale_Z = HMC5883_Scale_Z = (float)(HMC5883_maxx - HMC5883_minx)/(float)(HMC5883_maxz - HMC5883_minz);
